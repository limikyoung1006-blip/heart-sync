...
const ChatView = ({ userRole, husbandInfo, wifeInfo, onBack }) => {
  ...
  const hatti = useMemo(() => {
    ...
    return {
      ...
      intro: `안녕하세요! 저는 부부상담 전문가 AI하티라고 합니다. ${partnerLabel}분의 ${partnerInfo.mbti} 성향과 신앙의 조언을 담아 두 분의 마음을 이어드릴게요.`,
      partnerInfo,
      partnerLabel,
      color: '#8A60FF',
  ...
  const send = () => {
    ...
    setTimeout(() => {
      ...
      if (userRole === 'husband') {
        const responses = [
          `아내분의 ${p.mbti} 기질은 주님이 주신 특별한 생동감의 표현이에요. 형제님이 조금 무겁게 느껴지실 수도 있지만, 그리스도께서 교회를 위해 자신을 주심같이 그 다름을 먼저 안아주시면 하나님의 평강이 가정에 가득할 거예요.`,
          `${p.mbti} 아내분께는 공감이 세상에서 가장 큰 선물입니다. 오늘 형제님의 작은 인정 한마디가 아내분을 기도로 춤추게 할 수 있어요. 주님이 우리를 용납하셨듯 아내의 마음을 믿음으로 용납해 보세요.`,
          `아내분의 ${p.blood}형 특유의 섬세함과 ${p.mbti} 열정이 만날 때는 조금 더 세심한 보살핌이 필요할 수 있어요. 오늘 형제님이 던지는 따뜻한 격려가 아내 마음의 성벽을 허무는 은혜가 될 거예요.`
        ];
        response = responses[Math.floor(Math.random() * responses.length)];
      } else {
        const responses = [
          `남편분의 ${p.mbti} 기질은 묵묵한 순종과 성실함으로 사랑을 증명하는 방식이에요. 가끔 무뚝뚝해 보여도 주님이 맡기신 가정이라는 제단을 지키려는 그분의 노력을 이해해 주세요. 롬 8:28처럼 그 성실함 안에 주님의 뜻이 있습니다.`,
          `${p.mbti} 남편분은 결과보다 자매님의 신뢰를 먹고 깊어집니다. "당신을 믿어, 고생했어"라는 한마디가 남편분께는 주님의 은혜처럼 들릴 수 있어요. 오늘 그 마음을 다독여 주시는 건 어떨까요?`,
          `남편분의 ${p.blood}형 분석력과 ${p.mbti} 성향은 가문의 기둥처럼 단단하지만, 때론 자매님의 따뜻한 미소가 그 기둥을 더 아름답게 장식합니다. 먼저 다가가 손을 잡아주는 자매님의 용기가 가정을 주님의 뜰로 가꿀 거예요.`
        ];
        response = responses[Math.floor(Math.random() * responses.length)];
      }
