...
const GameGuideView = ({ gameId, onStart, onBack }) => {
  ...
  return (
    <motion.div 
      ...
      style={{ padding: '20px', minHeight: '100vh', background: isImageGame ? '#FDFCF0' : '#FDFCF0', display: 'flex', flexDirection: 'column' }}
    >
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
        <div style={{ 
          ...
          {isImageGame ? <Sparkles size={50} color="#AB47BC" /> : <BookOpen size={50} color="#D4AF37" />}
        </div>

        <h2 style={{ fontSize: '28px', fontWeight: 900, color: '#2D1F08', marginBottom: '15px' }}>
        ...
        <div style={{ width: '100%', background: 'white', borderRadius: '30px', padding: '30px', boxShadow: '0 10px 30px rgba(0,0,0,0.03)', textAlign: 'left', display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <h3 style={{ fontSize: '15px', fontWeight: 900, color: '#2D1F08', opacity: 0.5, letterSpacing: '1px' }}>GUIDE & RULES</h3>
          
          <div style={{ display: 'flex', gap: '15px', alignItems: 'flex-start' }}>
            <div style={{ width: '40px', height: '40px', borderRadius: '12px', background: isImageGame ? '#F3E5F5' : '#FFFDE7', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              {isImageGame ? <Infinity size={22} color="#AB47BC" /> : <Zap size={22} color="#D4AF37" />}
            </div>
            <div>
              <h4 style={{ fontSize: '16px', fontWeight: 900, color: '#2D1F08', marginBottom: '4px' }}>{isImageGame ? "두 가지 플레이 모드" : "5가지 대화 카테고리"}</h4>
              <p style={{ fontSize: '14px', color: '#8B7355', fontWeight: 600, lineHeight: 1.5 }}>
                {isImageGame 
                  ? "무작위 이미지를 확인하거나, 10장의 이미지 중\n현재 내 기분을 가장 잘 나타내는 2장을 골라보세요."
          ...
          <div style={{ display: 'flex', gap: '15px', alignItems: 'flex-start' }}>
            ...
            <div>
              <h4 style={{ fontSize: '16px', fontWeight: 900, color: '#2D1F08', marginBottom: '4px' }}>{isImageGame ? "실시간 동기화" : "대화의 완성 (10카드)"}</h4>
              <p style={{ fontSize: '14px', color: '#8B7355', fontWeight: 600, lineHeight: 1.5 }}>
                {isImageGame
      ...
      <div style={{ marginTop: '40px', paddingBottom: '30px' }}>
        <button 
          ...
          style={{ 
            ...
            color: 'white', fontWeight: 900, fontSize: '18px', border: 'none',
