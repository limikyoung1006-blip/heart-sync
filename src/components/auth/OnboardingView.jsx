        ...
        {step === 1 && (
          <motion.div key="step1" initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -20, opacity: 0 }} style={{ display: 'flex', flexDirection: 'column', height: '100%', justifyContent: 'center' }}>
            <div style={{ marginBottom: '40px', textAlign: 'left', paddingLeft: '10px' }}>
              ...
              <div style={{ padding: '4px 8px', borderRadius: '6px', background: 'rgba(138, 96, 255, 0.1)', color: '#8A60FF', fontSize: '9px', fontWeight: 900, display: 'inline-block', marginBottom: '20px' }}>v1.2.3 - ISOLATION TEST</div>
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      const finalPayload = {
                        ...
                        // info: JSON.stringify(sanitizedInfo), // 🛡️ ISOLATION TEST: Remove to see if error persists
                        updated_at: new Date().toISOString()
                      };
